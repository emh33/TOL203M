...
// Hnútar veggja
var vertices = [
    vec4( -0.5,  0.0, -20.0, 1.0 ),
    vec4( -0.5,  0.0, 20.0, 1.0 ),
    vec4( -0.5,  1.0, 20.0, 1.0 ),
    vec4( -0.5,  1.0, 20.0, 1.0 ),
    vec4( -0.5,  1.0, -20.0, 1.0 ),
    vec4( -0.5,  0.0, -20.0, 1.0 ),
...
window.onload = function init() {
    ...
    // Event listener for keyboard
     window.addEventListener("keydown", function(e){
         switch( e.keyCode ) {
            case 87:	// w
            if(userZPos >= -19.5 &&  userZPos <=19.5 ){ // enda veggir
                if(userXPos >= - 0.2 && userXPos <=  0.2 ){ // hliðar veggir
                ...
                break;
            case 83:	// s
            if(userZPos <= 15 && userZPos  >= -15){
                if(userXPos <=  0.2 && userXPos >= - 0.2){
                userXPos -= userIncr * userXDir;
                userZPos -= userIncr * userZDir;
                }
            }
                break;
            case 65:	// a
            if(userXPos +(userIncr * userZDir)>= - 0.4 && userXPos +(userIncr * userZDir)<=  0.4 ){
...
var render = function(){
    ...
    gl.bindTexture( gl.TEXTURE_2D, texGolf );
    gl.drawArrays( gl.TRIANGLES, numVertices, numVertices * 4 );
