...
function createCirclePoints( cent, rad, k )
{
    ...
    var p = vec2( rad*Math.sin(k*dAngle) + cent[0], rad*Math.cos(k*dAngle) + cent[1] );
    points.push(p);

    for( i=k-2; i>=0; i=i-2) {
        a = i*dAngle;
    	p = vec2( rad*Math.sin(a) + cent[0], rad*Math.cos(a) + cent[1] );
    	points.push(p,center,p);
    }
}
...
function render() {
    ...
    gl.drawArrays( gl.TRIANGLES, 0, numCirclePoints+numCirclePoints/2);
